-- Australian financial year start year for a given date (FY runs 1 Jul – 30 Jun).
...
CREATE TABLE IF NOT EXISTS public.business_settings (
  ...
  abn text, address text, phone text, email text,
...
CREATE TABLE IF NOT EXISTS public.document_counters (
  doc_type text NOT NULL, fy integer NOT NULL, last_number integer NOT NULL DEFAULT 0,
...
BEGIN
  ...
  VALUES (p_doc_type, v_fy, 0) ON CONFLICT (doc_type, fy) DO NOTHING;
  ...
  SELECT CASE p_doc_type WHEN 'quote' THEN coalesce(quote_prefix,'Q')
           WHEN 'invoice' THEN coalesce(invoice_prefix,'INV')
           ELSE upper(substr(p_doc_type,1,3)) END
    INTO v_prefix FROM public.business_settings LIMIT 1;
  RETURN coalesce(v_prefix, upper(substr(p_doc_type,1,3)))
         || '-' || v_fy::text || '-' || lpad(v_num::text, 3, '0');
END; $$;
...
CREATE TABLE IF NOT EXISTS public.quotes (
  ...
  customer_address text, suburb text,
  status text NOT NULL DEFAULT 'draft'
    CHECK (status IN ('draft','sent','accepted','declined','expired','invoiced')),
  issue_date date NOT NULL DEFAULT current_date, expiry_date date,
  subtotal numeric(12,2) NOT NULL DEFAULT 0,
  gst_amount numeric(12,2) NOT NULL DEFAULT 0,
  total numeric(12,2) NOT NULL DEFAULT 0,
  discount_percent numeric(5,2) NOT NULL DEFAULT 0,
  discount_amount  numeric(12,2) NOT NULL DEFAULT 0,
  customer_notes text, internal_notes text, pdf_url text,
  sent_at timestamptz, accepted_at timestamptz,
  xero_quote_id text, xero_contact_id text,
...
CREATE TABLE IF NOT EXISTS public.invoices (
  ...
  customer_notes text, internal_notes text, pdf_url text, sent_at timestamptz,
  xero_invoice_id text, xero_contact_id text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.expenses (
  ...
  amount numeric(12,2) NOT NULL DEFAULT 0, gst_amount numeric(12,2),
  supplier text, receipt_url text, tax_deductible boolean NOT NULL DEFAULT true,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
DO $$ DECLARE t text; BEGIN
  FOREACH t IN ARRAY ARRAY['quotes','quote_items','invoices','invoice_items','expenses','messages'] LOOP
...
CREATE POLICY "Admins manage documents" ON storage.objects FOR ALL TO authenticated
...
CREATE TABLE IF NOT EXISTS public.xero_connection (
  ...
  tenant_id text, tenant_name text, connected_at timestamptz,
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.xero_oauth_state (
  state text PRIMARY KEY, created_at timestamptz NOT NULL DEFAULT now()
);
